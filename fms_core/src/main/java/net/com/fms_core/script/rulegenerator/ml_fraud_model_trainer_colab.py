"""
ML-Based Fraud Detection Model Trainer - Colab Version
...
class MLFraudModelTrainer:
    def __init__(self, csv_directory="/content"):
    ...
    def load_and_preprocess_data(self):
        ...
        # Look for CSV files in /content directory
        csv_files = [
            'creditcard.csv',
        ...
        # Also check for any CSV files in /content
        all_files = os.listdir(self.csv_directory)
        csv_files_found = [f for f in all_files if f.endswith('.csv')]
        print(f"All CSV files found: {csv_files_found}")
        
        # Load additional CSV files not already loaded
        loaded_files = [df['source_file'].iloc[0] for df in all_data if len(df) > 0]
        
        for csv_file in csv_files_found:
            if csv_file not in loaded_files:
                file_path = os.path.join(self.csv_directory, csv_file)
                try:
                    print(f"Loading additional CSV: {csv_file}...")
                    df = pd.read_csv(file_path)
                    df['source_file'] = csv_file
                    all_data.append(df)
                    print(f"Loaded {len(df)} records from {csv_file}")
...
def main():
    # Use /content as default directory for Colab
    csv_directory = "/content"
    
    print("=" * 70)
    print("ML FRAUD DETECTION MODEL TRAINER - COLAB VERSION")
